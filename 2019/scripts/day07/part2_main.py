...
def intcode(mem, index=0, input_list=(), debug=False):
...
def main(amp_code):
    ...
    computers = []
    for i in amp_code:
        com = [[i], mem_input.copy(), 0]
        computers.append(com)

    last_output = 0
    i = 0
    while True:
        code, int_mem, pos = computers[i]

        input_list = code + [last_output]
        output, mem, index = intcode(int_mem, index=pos, debug=False, input_list=input_list)
        if output:
            last_output = output
        if not mem:
            break
